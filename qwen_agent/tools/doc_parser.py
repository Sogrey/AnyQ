...
class DocParser(BaseTool):
    ...
    def call(self, params: Union[str, dict], **kwargs) -> dict:
        ...
        try:
            ...
            logger.info(f'Read chunked {os.path.basename(url)} from cache.')
            return record
        ...
        logger.info(f'Start chunking ({title})...')
        time1 = time.time()
        ...
        time2 = time.time()
        logger.info(f'Finished chunking ({title}). Time spent: {time2 - time1} seconds.')
