...
你总是用中文回复用户。'''
    ...
    # 配置ESMemory用于缓存
    # 尝试使用ESMemory，如果失败则回退到不使用memory
    try:
    ...
    # 保存ESMemory实例到会话状态，以便后续可能使用
    if memory:
        st.session_state.es_memory = memory
    ...
    if 'processing_response' in st.session_state and st.session_state.processing_response and len(st.session_state.messages) > 0:
        ...
        with col_main:            
            ...
            if last_user_msg['role'] == 'user':
                st.markdown(f'<div class="user-message"><strong>用户:</strong> {last_user_msg["content"]}</div>', unsafe_allow_html=True)
            ...
            if hasattr(st.session_state.bot, 'memory') and st.session_state.bot.memory:
                if hasattr(st.session_state.bot.memory, 'function_map'):
                    if 'streamlit_es_vector_search' in st.session_state.bot.memory.function_map:
                        retrieved_docs = st.session_state.bot.memory.function_map['streamlit_es_vector_search'].call({'query': user_message_content})
                        if retrieved_docs:
